...
# Rscript ./test_scripts/ratio_analysis.R -A single_gene --genes "TP53 EGFR" --datasets "TCGA-ACC TCGA-LGG" --tumor_type primary -S overall --group_method median
...
if (TEST){
    args = list(
    options = list(
        session_id="tmp_ratio_",
        analysis_type="gene_ratio",
        genes="TP53 EGFR",
...
if(length(sig_genes)<2){
    write("[IT006] Only one/None gene is provided for gene ratio analysis; Two are required.\n", stderr())
    stop()
}
...
for(i in 1:num_ds){ ## allow upto five datasets
    ...
    gene_fpkm_A <- fpkm[fpkm$symbol == gene_A, ]
    gene_fpkm_B <- fpkm[fpkm$symbol == gene_B, ]
...
for (ds in gene_datasets){
    ...
    ds_figure[["waterfall"]] = (waterfallplot(scores_df[[ds]],ds,"Log2 Genes Expression Ratio",COL_GROUPS) + theme_classic()) |> plotly2json()
    ds_figure[["boxplot"]] = (generate_boxplot_gg(scores_df[[ds]],ds,label="Log2 Genes Expression Ratio",COL_GROUPS) + theme_classic()) |> plotly2json()
