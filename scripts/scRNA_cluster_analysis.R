...
# Rscript ./test_scripts/scRNA_cluster_analysis.R -A scRNA --input_type example_data --datasets TCGA-ACC --tumor_type primary -S overall --group_method median
...
if (TEST){
    args = list(
    options = list(
        session_id="tmp_sccluster_",
        analysis_type="cluster_marker",
...
if(args$options$input_type == "example_data"){ #if an example gene set
    dataFile <- paste(FILE_DIR, CLUSTER_DIR, "cluster-markers.txt", sep='/')
    if (!file.exists( dataFile) ){
        write("[IT007] Cluster marker file does not exist.\n", stderr())
        stop()
    }
    ...
    if(httr::HEAD(dataFile)$status_code != 200){
        write("[IT007] Cluster marker file url cannot be loaded.\n", stderr())
        stop()
    }
...
if((!("p_val_adj" %in% colnames(sig_list))) || 
   (!("gene" %in% colnames(sig_list))) ||
   (!("cluster" %in% colnames(sig_list)))
    ){
...
if (args$options$gene_filter == "lnc") {
    ...
    ref = read.table(paste(REF_DIR, "approved_symbols.txt", sep='/'),header=F)[["V1"]]
    sig_list <- subset(sig_list, sig_list$gene %in% ref)
}

clusters <- unique(as.character(sig_list$cluster))
num_cls <- length(clusters)
if(num_cls == 0 || any(dim(sig_list) == 0)){
    write("[IT008] Cluster markers are empty after filtering p_adj<0.05 and Gene Filter.\n", stderr())
    stop()
}
...
cutp_vec <- rep("", num_cls)
names(cutp_vec) <- clusters
lrpval <- vector("list",length=num_cls)
names(lrpval) <- clusters
scores_df <- vector("list",length=num_cls)
names(scores_df) <- clusters
input_df <- vector("list",length=num_cls)
names(input_df) <- clusters
km_models <- vector("list",length=num_cls)
names(km_models) <- clusters


cox.tbl = data.frame(matrix(vector(), num_cls+2, 10,
        dimnames=list(c(),c("Dataset", "HR_Ratio","HR_CI_low", "HR_CI_high", "nlow", "nhigh", "HR", "HR_Pvalue", "Cut_Point", "LR_Pvalue"))))

res <-c("Cluster", "HR", "HR", "HR", "nLow", "nHigh", "HR", "HR", "Point", "LR")
cox.tbl[1,] <- res
...
for(i in 1:num_cls){ ## allow upto five datasets

    sig_genes <- as.character(sig_list$gene[sig_list$cluster==clusters[i]])
    ...
    res <-c(as.character(clusters[i]), HR.ratio, HR.confint.lower, HR.confint.upper, nlow, nhigh, HR, hrpval, paste0(low_cpi,",", high_cpi), lr_pval)
    cox.tbl[2+i,] <- res
    rownames(cox.tbl)[2+i] <- as.character(clusters[i])
...
# return_data=list(scores_df=scores_df, cpi=cutp_vec, input_df=input_df, lrpval=lrpval, cox.tbl=cox.dt.df, n=num_cls, gene_datasets=gene_datasets)
# km_models
...
figures <- vector("list",length=num_cls+4)
names(figures) <- c(clusters,"analysis_type","hrplot", "forestplot","signature_corr")
...
for (ds in clusters){
    ...
    ds_figure[["waterfall"]] = (waterfallplot(scores_df[[ds]],ds,"Enrichment Score",COL_GROUPS) + theme_classic()) |> plotly2json()
    ds_figure[["boxplot"]] = (generate_boxplot_gg(scores_df[[ds]],ds,label="Enrichment Score",COL_GROUPS) + theme_classic()) |> plotly2json()
...
corr_score_df <- vector("list",length=length(clusters))
names(corr_score_df) = clusters
for (cls in clusters){
    gx_scores <- scores_df[[cls]][,c("sample", "gx")]
    colnames(gx_scores) <- c("sample", cls)
    corr_score_df[[cls]] = gx_scores
}
...
#subset samples by CIBERSOFT p-value <=0.05
    ...
    p.coef_p <- p.coef$p[1:num_cls,-c(1:num_cls)]
